"""Evaluación del dataset ESCRITURAS
Basado en el "Official evaluation script for SQuAD version 2.0."
...
def normalize_answer(s):
    ...
    def remove_articles(text):
        regex = re.compile(r"\b(un|una|unos|unas|el|la|lo|las|los)\b", re.UNICODE)
...
def get_raw_scores_by_example(examples):
    f1_scores = {}
    exact_scores = {}
    for qid, gold, pred in zip(examples['id'], examples['answers'],examples['prediccion']):
        f1_scores[qid] = max(compute_f1(gold_answer,pred['answer']) for gold_answer  in gold['text'])
        exact_scores[qid] = max(compute_exact(gold_answer,pred['answer']) for gold_answer  in gold['text'])
    return exact_scores, f1_scores

def get_raw_scores_by_prediction(dataset,predictions):
    f1_scores = {}
    exact_scores = {}
    for qid, gold_list, pred in zip(dataset['id'], dataset['answers'], predictions):
        f1_scores[qid]=(max(compute_f1(gold_answer,pred['answer']) for gold_answer  in gold_list['text']))
